...
select * 
from article as a 
WHERE upper(a.designation) like '%SACHET%';
--f
SELECT * 
FROM article 
...
--h
SELECT * AVG(article.prix) as prix_moyen, fournisseur.nom 
...
WHERE a.designation like 'Boulon%' 
--m
SELECT b.id,b.numero,b.date_cmde,b.delai, sum(c.qte*a.prix)  as 'prix du bon de commande'
FROM compo as c 
inner join article as a on c.id_art = a.id 
inner join bon as b on c.id_bon=b.id
group by b.id;
--n
SELECT b.id,b.numero,b.date_cmde,b.delai, sum(c.qte)  as 'prix du bon de commande'
FROM compo as c 
inner join bon as b on c.id_bon=b.id
group by b.id;
--o
SELECT b.id, sum(c.qte)  as 'qte'
FROM compo as c 
inner join bon as b on c.id_bon=b.id
group by b.id
having qte>25;
